...
export default class CalendarDayScreen extends React.Component {
  ...
  async loadAudio() {
    if (this.state.playbackInstance === null) {
      ...
      await this.state.playbackInstance.loadAsync(
        require('../assets/sounds/early_morning_fog.mp3'),
      );
      await this.state.playbackInstance.setOnPlaybackStatusUpdate(
  ...
  async setTime() {
    let minutes = parseInt(
      (this.state.playbackInstanceDuration / (1000 * 60)) % 60,
    );
    if (minutes < 10) {
    ...
    await this.setState({
      totalTime: {
        minutes: minutes,
        seconds: parseInt(
          (this.state.playbackInstanceDuration / 1000) % 60,
        ).toString(),
      },
    });
  }
  ...
  async setCurrentTime() {
    let minutes = parseInt(
      (this.state.playbackInstancePosition / (1000 * 60)) % 60,
    );
